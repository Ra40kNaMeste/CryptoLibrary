...
template <int size_block, int count_block, int size_key, int size_round_key>
void FeistelNetwork<size_block, count_block, size_key,
                    size_round_key>::encryption(const char *input,
                                                char *output) {
...
template <int size_block, int count_block, int size_key, int size_round_key>
void FeistelNetwork<size_block, count_block, size_key,
                    size_round_key>::decryption(const char *input,
                                                char *output) {
...
template <int size_block, int count_block, int size_key, int size_round_key>
void FeistelNetwork<size_block, count_block, size_key,
                    size_round_key>::cryption(const char *input, char *output,
                                              std::function<int(int)> get_i) {
  char *buf = new char[size_block*count_block];
  std::memcpy(buf, input, size_block*count_block);
  int i = 0;
  ...
  i %= count_block;
  std::memcpy(output, buf + i * size_block, (count_block - i) * size_block);
  std::memcpy(output + (count_block - i) * size_block, buf, i * size_block);
  for (int i = 0; i < size_block*count_block; i++) {
    buf[i] = 0;
  }
